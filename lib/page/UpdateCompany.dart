...
class UpdateCompanyPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _UpdateCompanyPage();
}

class _UpdateCompanyPage extends State<UpdateCompanyPage> {
  ...
  File? logoFile;
  File? licenseFile;
  String? logoUrl; // URL từ backend
  String? licenseUrl; // URL từ backend
  ...
  Future<void> _loadProfile() async {
    final data = await _employerService.getCompany();
    if (data != null) {
      setState(() {
        ...
        companyDesciption.text = data['companyDescription'] ?? '';
        selectedLocation = LatLng(
          (data['latitude'] ??0 )/10000000,
          (data['longitude'] ??0 )/1000000,
        );
        print('Latitude: ${selectedLocation.latitude}, '
            'Longitude: ${selectedLocation.longitude}');
        logoUrl = data['companyLogo'];
        licenseUrl = data['licenseDocument'];
        _isLoading = false;
      });
    } else {
    ...
    Future<void> submit() async {
      ...
      final updateEmployer = await _employerService.updateEmployer(
        data,
        logoFile,
        licenseFile,
      );
      if (updateEmployer != null) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Cập nhật công ty thành công!")));
        Navigator.pop(context);
      } else {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Cập nhật công ty thất bại!")));
        print("id ${_profileId}");
      }
    }
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(width: 10),
                      (logoFile != null  || logoUrl !=null)? Text("Đã chọn") : Text("Chưa chọn"),
                    ],
                  ),
                  Row(
                    children: [
                      ...
                      SizedBox(width: 10),
                      (licenseFile != null || licenseUrl!=null)
                          ? Text("Đã chọn")
                          : Text(
                              "Chưa chọn",
                            ), // <-- sửa logoFile thành licenseFile
                    ],
                  ),
                  if (!_isLoading && selectedLocation != 0)
                  SizedBox(height: 20),
                  Container(
                    height: 250,
